...
namespace BetfairStreamClient.tests.ExchangeStream.Cache
{
    ...
    public class Market
    {
        ...
        private Model.MarketDefinition _marketDefinition;
        private double _tv;
        ...
        internal void OnMarketChange(MarketChange marketChange)
        {
            ...
            Cache.MarketSnap newSnap = new Cache.MarketSnap();
        ...
        private void OnPriceChange(bool isImage, Model.RunnerChange runnerChange)
        {
        ...
        private void OnMarketDefinitionChange(Model.MarketDefinition marketDefinition)
        {
            _marketDefinition = marketDefinition;
            if (marketDefinition.Runners != null)
            {
                foreach (Model.RunnerDefinition runnerDefinition in marketDefinition.Runners)
                {
        ...
        private void OnRunnerDefinitionChange(Model.RunnerDefinition runnerDefinition)
        {
        ...
        public bool IsClosed
        {
            get
            {
                return _marketDefinition != null && _marketDefinition.Status == Model.MarketDefinition.StatusEnum.Closed;
            }
        }
